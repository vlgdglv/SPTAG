...
public:
    ...
    int LoadingHeadInfo(std::shared_ptr<Helper::DiskIO>& ptr, std::vector<ListInfo>& m_listInfos)
    {
        int m_listCount;
...
ErrorCode ParsePosting() {
    ...
    if ((ret = VectorIndex::LoadIndex(options.m_indexFolder + FolderSep + options.m_headIndex, headIndex)) != ErrorCode::Success) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Cannot load index %s\n", options.m_indexFolder.c_str());
        return ret;
    }
    ...
    if (ptr == nullptr || !ptr->Initialize((options.m_indexFolder + FolderSep + options.m_output).c_str(), std::ios::out)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Cannot open file %s to write!", (options.m_indexFolder + FolderSep + options.m_output).c_str());
        return ErrorCode::Fail;
    }
    ...
    if (metaIndex == nullptr) SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Output vector ids\n");
    else SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Output vector metas\n");
    ...
    for (int curPostingID = 0; curPostingID < headIndex->GetNumSamples(); curPostingID++) {
        ...
        Helper::DiskIO* indexFile = tailIndex.m_indexFiles[fileid].get();

...
#pragma omp parallel for schedule(dynamic)
            ...
            void ShowListings()
            {
                for (int i = 0; i < m_listInfos.size(); i++)
                {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "List %d: eleCount=%d, listPageCount=:%d\n", i, m_listInfos[i].listEleCount, m_listInfos[i].listPageCount);
                }
            }
